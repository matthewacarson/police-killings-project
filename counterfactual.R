...
summary_tables$race_LUOF_counterfactual$rate <-
  summary_tables$race_LUOF_counterfactual$LUOF_count_quint_race /
  summary_tables$race_LUOF_counterfactual$pop_quint_race * 10000000 / 6

# Subset tracts where whites were killed -- quintiles
summary_tables$race_LUOF_counterfactual_White <-
  summary_tables$race_LUOF_counterfactual[summary_tables$race_LUOF_counterfactual$race == "White",]
...
summary_tables$race_LUOF_counterfactual_POC$white_prop_in_tract <-
  summary_tables$race_LUOF_counterfactual_White$prop_of_race_in_quintile[match(
    summary_tables$race_LUOF_counterfactual_POC$quintile,
    summary_tables$race_LUOF_counterfactual_White$quintile
  )]

# Proportion of the race of victim in that tract income quintile
summary_tables$race_LUOF_counterfactual_POC$`prop_of_race_in_quintile*rate` <-
  summary_tables$race_LUOF_counterfactual_POC$prop_of_race_in_quintile * 
  summary_tables$race_LUOF_counterfactual_POC$rate
...
summary_tables$race_LUOF_counterfactual_POC$counterfactual <-
  summary_tables$race_LUOF_counterfactual_POC$rate *
  summary_tables$race_LUOF_counterfactual_POC$white_prop_in_tract

# Rounding the results for display (excluding non-numeric columns)
summary_tables$race_LUOF_counterfactual_rounded <-
  apply(summary_tables$race_LUOF_counterfactual_POC[, 3:12], MARGIN = 2,
        FUN = function(x) round(x = x, digits = 2)) |> as.data.frame()

# Rejoining non-numeric columns
summary_tables$race_LUOF_counterfactual_rounded <-
  cbind(summary_tables$race_LUOF_counterfactual_POC[, 1:2],
        summary_tables$race_LUOF_counterfactual_rounded
)
...
mean(summary_tables$race_LUOF_counterfactual_White$rate)
...
# counterfactual rates are much lower than the white rate.
